...
paste. If your VM doesn't have a `~/.ssh/authorized_keys` file, create it. It's
important that you do this for the user you made on the VM, and **not** the
`root` user, and also that only the user has permissions to read their `~/.ssh`:

{% highlight bash %}
chmod 700 ~/.ssh
{% endhighlight %}

You should now have set up everything that's required on the VM's side of
things. Make sure you have it shut down for the following sections.
